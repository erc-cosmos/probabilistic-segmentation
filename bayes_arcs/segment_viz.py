...
def plot_segment_raindrop(data, max_length, ratio=4, show=True):
    ...
    im = ax.imshow(np.transpose(linear_viz_point), cmap=plt.get_cmap('hot'), origin='lower', aspect=ratio)
    cbar = fig.colorbar(im)
    cbar.set_label("Likelihood")
...
def plot_segment_beams(post2_bidim, max_length, ratio=4, show=True):
    ...
    im = ax.imshow(np.transpose(linear_viz), cmap=plt.get_cmap('hot'), origin='lower', aspect=ratio)
    cbar = fig.colorbar(im)
    cbar.set_label("Likelihood")
...
def plot_segment_with_signal(post_marginals, data, data_time, boundaries=None, *,
    ...
    lines = line_data + line_proba
    labels = [input_label, 'likelihood of boundary']
