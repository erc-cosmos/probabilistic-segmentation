...
def writeMeasures(filename, measures):
    ...
    with open(filename,'w') as csvFile: 
        writer = csv.writer(csvFile)
        writer.writerow(["feature", "piece", "performer", "", "Quadratic/Brier score", "F1-score", "Recall", "Precision"])
        rows = [ list(key.split('/')) + list(scores) for key, scores in measures.items()]
